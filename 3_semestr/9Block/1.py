...
# Замените путь на свой, если нужно
...
# поэтому отдельно масштабируем данные для неё.
...
# =========================
# 4. Лаба 9.1 — три базовые модели
# =========================

# ---------- 4.1. Дерево решений (Decision Tree) ----------

# Простое дерево решений с разумными ограничениями по глубине
...
# ---------- 4.2. XGBoost (базовая настройка) ----------

xgb_clf = XGBClassifier(
    n_estimators=200,  # число деревьев
    max_depth=4,  # глубина базовых деревьев
    learning_rate=0.1,  # шаг обучения
    subsample=0.9,  # доля объектов для каждого дерева
...
# ---------- 4.3. Логистическая регрессия (Logistic Regression) ----------
...
# Используем обученное дерево tree_clf
...
class MyRandomForest:
    def __init__(
        ...
        # Максимальная глубина каждого дерева (None — без ограничений)
        self.max_depth = max_depth
        # Число признаков, используемых в каждом узле (sqrt — классический вариант)
        self.max_features = max_features
        # Критерий разбиения ("gini" или "entropy")
        self.criterion = criterion
        # Общий random_state для воспроизводимости
    ...
    def predict(self, X):
        ...
        X = np.asarray(X)
        if not self.trees:
            raise RuntimeError("Сначала нужно обучить лес с помощью fit().")

        # Предсказания всех деревьев: матрица формы (n_estimators, n_samples)
        ...
        for j in range(n_samples):
            # Берём предсказания всех деревьев для объекта j
